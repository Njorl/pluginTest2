...
function ShouldConfirmEventForTrackOnZoneOrTripwire(track,zoneInst,zoneId,tripwireInst,tripwireId)
    ...
    if ignoreStationaryObjects == true and IsTrackMoving(track) ~= true then
        return false
    end
    ...
    for _, track in pairs(tracks) do
        ...
        if requireLock ~= true or (requireLock == true and IsTrackLocked(track)) then
            ...
            if enableDebugSinks then
                bbox.label = bbox.label .. " ,m: " .. tostring(IsTrackMoving(track))
            end
...
function UpdateTracksMovementStatus(instance,tracks,trackerInst,currentTimeSec,inputSize)
    ...
    for _, track in pairs(tracks) do
        ...
        if math.abs(accumMovement.x) + math.abs(accumMovement.y) > relativeMovementTreshold then
            TrackMeta.setValue(trackId,TrackMetaKeys.IsMoving, true)
...
function IsTrackMoving(track)
    local isMoving = TrackMeta.getValue(track.id,TrackMetaKeys.IsMoving) == true
    return isMoving
end
...
---@return Event[], Event[]
function ProcessTentativeEvents(tentativeEvents, zoneInst, tripwireInst, currentTimeSec,allTracks)
        ...
        if zoneId ~= nil then
            ...
            if ((triggerOnEnter and eventType == EventTypes.AreaExit) or (triggerOnIntrusion and eventType == EventTypes.IntrusionEnd)) or eventType == EventTypes.LoiteringEnd then
                ...
                if matchingStartEventData == nil then
                    ...
                    event.extra = matchingStartEventData.extra
    ...
    for _, event in ipairs(confirmedEvents) do
        ProcessConfirmedEvent(event,zoneInst,tripwireInst,allTracks)
    end
...
---@param tripwireInst tripwiremanaged
---@param allTracks Track[]
function ProcessConfirmedEvent(event,zoneInst,tripwireInst,allTracks)
...
---@param tracks Track[]
---@param zoneInst zonemanaged
function UpdateEventStartDataEntries(tracks,zoneInst)

    local zoneIds = zoneInst:getZoneIds()

    for _, zoneId in ipairs(zoneIds) do

        local confirmedEnters = zoneInst:getZoneValue(zoneId,ZoneStateFieldPaths.ConfirmedEnters,ZoneDictType.State) or {}
        local confirmedIntrusions = zoneInst:getZoneValue(zoneId,ZoneStateFieldPaths.ConfirmedIntrusions,ZoneDictType.State) or {}
        local confirmedLoiterings = zoneInst:getZoneValue(zoneId,ZoneStateFieldPaths.ConfirmedLoiterings,ZoneDictType.State) or {}

        for trackId, _ in pairs(confirmedEnters) do
            local track = GetTrackById(tracks,trackId)
            if track ~= nil then
                zoneInst:setZoneValue(zoneId,ZoneStateFieldPaths.ConfirmedEnters.."/"..trackId.."/extra/bbox",track.bbox,ZoneDictType.State)
            end
        end

        for trackId, _ in pairs(confirmedIntrusions) do
            local track = GetTrackById(tracks,trackId)
            if track ~= nil then
                zoneInst:setZoneValue(zoneId,ZoneStateFieldPaths.ConfirmedIntrusions.."/"..trackId.."/extra/bbox",track.bbox,ZoneDictType.State)
            end
        end


        for trackId, _ in pairs(confirmedLoiterings) do
            local track = GetTrackById(tracks,trackId)
            if track ~= nil then
                zoneInst:setZoneValue(zoneId,ZoneStateFieldPaths.ConfirmedLoiterings.."/"..trackId.."/extra/bbox",track.bbox,ZoneDictType.State)
            end
        end
